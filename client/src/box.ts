...
export const getDataUnions = async () => {
	const unions = await Box.getThread(marketplaceId, DATA_UNIONS_THREAD, boxUnionsModerator, false)
...
export const createDataUnion = async (space: any, unionId: string, unionData: IDataUnion) => {
	const unionsRegistry = await space.joinThread(DATA_UNIONS_THREAD)
	const unions = await unionsRegistry.getPosts()
	if (!unions.find((union: any) => union.message.legalName.value.toLowerCase() === unionData.name.toLowerCase())) {
		console.log('creating new Data Union')
		const union = await space.joinThread(unionId)
		const unionSchema = getDataUnionSchema(Object.assign(unionData, { address: union._address }))
